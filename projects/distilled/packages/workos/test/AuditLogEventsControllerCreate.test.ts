...
describe("AuditLogEventsControllerCreate", () => {
  it("creates an audit log event for an action with a registered schema", async () => {
  ...
  it("fails with NotFound when the organization does not exist", async () => {
  ...
  it("fails with UnprocessableEntity when the event references an unregistered action", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const org = yield* OrganizationsControllerCreate({
  ...
  it("fails with BadRequest when occurred_at is not a valid date", async () => {
