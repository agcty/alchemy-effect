...
describe("AuthorizationControllerListResourcesForMembership", () => {
  it("lists resources for an organization membership", async (ctx) => {
  ...
  it("fails with BadRequest when neither parent_resource_id nor parent_resource_external_id+slug are provided", async () => {
  ...
  it("fails with Forbidden when the membership belongs to a different tenant", async () => {
    const error = await runEffect(
      AuthorizationControllerListResourcesForMembership({
  ...
  it("fails with UnprocessableEntity when both parent_resource_id and parent_resource_external_id are provided", async () => {
    const error = await runEffect(
      AuthorizationControllerListResourcesForMembership({
        organization_membership_id: `om_${testRunId}`,
        permission_slug: "read",
