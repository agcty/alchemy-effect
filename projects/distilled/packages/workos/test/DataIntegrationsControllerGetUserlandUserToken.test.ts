...
describe("DataIntegrationsControllerGetUserlandUserToken", () => {
  it("returns a token for a connected account or NotFound if not connected", async () => {
    ...
    const result = await runEffect(
      DataIntegrationsControllerGetUserlandUserToken({
        ...
        Effect.matchEffect({
          ...
          onSuccess: (value) => Effect.succeed({ kind: "ok" as const, value }),
        }),
      ),
    );
  ...
  it("fails with NotFound for a non-existent provider slug", async () => {
    const users = await runEffect(UserlandUsersControllerList({ limit: 1 }));
    const userId =
      users.data.length > 0
  ...
  it("fails with BadRequest when user_id is malformed", async () => {
    const error = await runEffect(
      DataIntegrationsControllerGetUserlandUserToken({
        slug: "github",
