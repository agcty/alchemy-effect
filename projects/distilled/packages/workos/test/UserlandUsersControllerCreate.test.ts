...
describe("UserlandUsersControllerCreate", () => {
  it("creates a user, or surfaces a typed error", async () => {
  ...
  it("fails with a typed NotFound when a referenced resource cannot be resolved", async () => {
    const error = await runEffect(
      UserlandUsersControllerCreate({}).pipe(Effect.flip),
    );
    expect(typedErrorTags).toContain(error._tag);
  ...
  it("fails with a typed UnprocessableEntity for semantically invalid user payloads", async () => {
    const error = await runEffect(
      UserlandUsersControllerCreate({}).pipe(Effect.flip),
    );
    expect(typedErrorTags).toContain(error._tag);
