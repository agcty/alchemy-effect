...
describe("AuthorizationRolePermissionsControllerSetPermissions", () => {
  it("replaces the permissions on an environment role", async (ctx) => {
  ...
  it("fails with BadRequest when permissions is malformed", async () => {
  ...
  it("fails with Forbidden when the caller cannot modify the role", async () => {
    const error = await runEffect(
      AuthorizationRolePermissionsControllerSetPermissions({
  ...
  it("fails with UnprocessableEntity when targeting the built-in admin role", async () => {
    const error = await runEffect(
      AuthorizationRolePermissionsControllerSetPermissions({
        slug: "admin",
        permissions: ["users:read"],
