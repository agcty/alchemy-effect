...
describe("UserlandUsersControllerSendEmailChange", () => {
  it("sends an email change code, or surfaces a typed error", async () => {
  ...
  it("fails with a typed Conflict when the new email collides with an existing user", async () => {
  ...
  it("fails with a typed UnprocessableEntity for a malformed new_email", async () => {
    const users = await runEffect(UserlandUsersControllerList({ limit: 1 }));
    const seedId =
      users.data.length > 0
        ? (users.data[0] as { id: string }).id
