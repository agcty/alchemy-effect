...
describe("UserlandUsersControllerConfirmEmailChange", () => {
  it("confirms an email change, or surfaces a typed error", async () => {
  ...
  it("fails with a typed BadRequest when the code is empty", async () => {
    const users = await runEffect(UserlandUsersControllerList({ limit: 1 }));
    const seedId =
      users.data.length > 0
        ? (users.data[0] as { id: string }).id
  ...
  it("fails with a typed Conflict, BadRequest, or related error for an unrecognized code", async () => {
  ...
  it("fails with a typed UnprocessableEntity for a malformed code value", async () => {
    const users = await runEffect(UserlandUsersControllerList({ limit: 1 }));
    const seedId =
      users.data.length > 0
        ? (users.data[0] as { id: string }).id
