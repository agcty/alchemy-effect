...
describe("AuthorizationPermissionsControllerUpdate", () => {
  it("updates a permission's name and description", async () => {
  ...
  it("fails with Forbidden when updating a system permission", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
  ...
  it("fails with UnprocessableEntity when name is empty", async () => {
