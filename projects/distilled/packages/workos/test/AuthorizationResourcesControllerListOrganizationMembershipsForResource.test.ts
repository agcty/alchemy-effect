...
describe("AuthorizationResourcesControllerListOrganizationMembershipsForResource", () => {
  it("lists organization memberships for a resource", async (ctx) => {
  ...
  it("fails with BadRequest when permission_slug is missing", async () => {
    const error = await runEffect(
      AuthorizationResourcesControllerListOrganizationMembershipsForResource({
  ...
  it("fails with Forbidden when the resource is in a different tenant", async () => {
    const error = await runEffect(
      AuthorizationResourcesControllerListOrganizationMembershipsForResource({
  ...
  it("fails with UnprocessableEntity when the permission slug is malformed", async () => {
    const error = await runEffect(
      AuthorizationResourcesControllerListOrganizationMembershipsForResource({
        resource_id: `resource_${testRunId}`,
