...
describe("WidgetsPublicControllerIssueWidgetSessionToken", () => {
  it("issues a widget session token for an organization, or surfaces a typed error", async () => {
  ...
  it("fails with a typed BadRequest when scopes is empty", async () => {
    const orgs = await runEffect(OrganizationsControllerList({ limit: 1 }));
    const orgId =
      orgs.data.length > 0
  ...
  it("fails with UnprocessableEntity for a malformed organization id", async () => {
    const error = await runEffect(
      WidgetsPublicControllerIssueWidgetSessionToken({
