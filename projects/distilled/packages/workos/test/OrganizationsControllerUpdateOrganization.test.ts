...
describe("OrganizationsControllerUpdateOrganization", () => {
  ...
  it("fails with Forbidden when updating an organization in a different tenant", async () => {
    const error = await runEffect(
      OrganizationsControllerUpdateOrganization({
  ...
  it("fails with BadRequest for an empty name", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const created = yield* OrganizationsControllerCreate({
  ...
  it("fails with Conflict when updating to an already-used external_id", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
  ...
  it("fails with UnprocessableEntity for duplicate domains in domain_data", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const created = yield* OrganizationsControllerCreate({
