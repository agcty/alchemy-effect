...
describe("PortalSessionsControllerCreate", () => {
  it("generates a portal link for an organization", async () => {
  ...
  it("fails with BadRequest when organization is missing", async () => {
  ...
  it("fails with Forbidden when generating a link for a different tenant", async () => {
    const error = await runEffect(
      PortalSessionsControllerCreate({
  ...
  it("fails with UnprocessableEntity for an invalid contact email", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const org = yield* OrganizationsControllerCreate({
