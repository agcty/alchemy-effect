...
describe("SsoControllerLogoutAuthorize", () => {
  it("generates a logout token for a profile", async (ctx) => {
  ...
  it("fails with NotFound for a non-existent profile id", async () => {
    const error = await runEffect(
      SsoControllerLogoutAuthorize({
        profile_id: `prof_does_not_exist_${testRunId}`,
      }).pipe(Effect.flip),
  ...
  it("fails with BadRequest when profile_id is empty", async () => {
    const error = await runEffect(
