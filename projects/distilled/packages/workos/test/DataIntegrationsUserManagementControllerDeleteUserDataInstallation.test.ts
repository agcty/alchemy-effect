...
describe("DataIntegrationsUserManagementControllerDeleteUserDataInstallation", () => {
  it("deletes a user's connected account, or surfaces a typed NotFound", async () => {
    ...
    const result = await runEffect(
      DataIntegrationsUserManagementControllerDeleteUserDataInstallation(seed),
    );
  ...
  it("fails with NotFound for a non-existent user/provider pair", async () => {
    const error = await runEffect(
      DataIntegrationsUserManagementControllerDeleteUserDataInstallation({
