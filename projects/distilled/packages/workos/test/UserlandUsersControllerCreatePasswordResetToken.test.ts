...
describe("UserlandUsersControllerCreatePasswordResetToken", () => {
  ...
  it("fails with Forbidden when password reset is not permitted for the user", async () => {
  ...
  it("fails with NotFound for a non-existent email", async () => {
    const error = await runEffect(
      UserlandUsersControllerCreatePasswordResetToken({
        email: `distilled-no-such-user-${testRunId}@example.com`,
      }).pipe(Effect.flip),
  ...
  it("fails with UnprocessableEntity for a malformed email", async () => {
    const error = await runEffect(
      UserlandUsersControllerCreatePasswordResetToken({
