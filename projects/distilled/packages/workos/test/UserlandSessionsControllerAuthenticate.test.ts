...
describe("UserlandSessionsControllerAuthenticate", () => {
  it("authenticates a user and returns a session, or surfaces a typed error", async () => {
  ...
  it("fails with a typed Forbidden when the caller is not permitted to authenticate", async () => {
    const error = await runEffect(
      UserlandSessionsControllerAuthenticate({}).pipe(Effect.flip),
    );
    expect(typedErrorTags).toContain(error._tag);
  ...
  it("fails with a typed NotFound when the referenced auth context cannot be resolved", async () => {
    const error = await runEffect(
      UserlandSessionsControllerAuthenticate({}).pipe(Effect.flip),
    );
    expect(typedErrorTags).toContain(error._tag);
  ...
  it("fails with a typed UnprocessableEntity for semantically invalid auth payloads", async () => {
    const error = await runEffect(
      UserlandSessionsControllerAuthenticate({}).pipe(Effect.flip),
    );
    expect(typedErrorTags).toContain(error._tag);
