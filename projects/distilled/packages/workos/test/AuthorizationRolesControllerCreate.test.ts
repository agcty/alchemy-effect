...
describe("AuthorizationRolesControllerCreate", () => {
  it("creates a new environment role", async () => {
  ...
  it("fails with BadRequest when required fields are missing", async () => {
  ...
  it("fails with NotFound when the resource_type_slug does not exist", async () => {
    const error = await runEffect(
      AuthorizationRolesControllerCreate({
  ...
  it("fails with Forbidden when targeting a resource_type in a different tenant", async () => {
    const error = await runEffect(
      AuthorizationRolesControllerCreate({
  ...
  it("fails with Conflict when creating a role with an existing slug", async () => {
  ...
  it("fails with UnprocessableEntity when the slug is malformed", async () => {
    const error = await runEffect(
      AuthorizationRolesControllerCreate({
