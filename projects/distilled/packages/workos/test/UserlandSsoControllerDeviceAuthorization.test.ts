...
describe("UserlandSsoControllerDeviceAuthorization", () => {
  it("issues a device code and verification URL", async (ctx) => {
  ...
  it("fails with BadRequest when client_id is empty", async () => {
  ...
  it("fails with Forbidden when client_id belongs to another environment", async () => {
    const error = await runEffect(
      UserlandSsoControllerDeviceAuthorization({
