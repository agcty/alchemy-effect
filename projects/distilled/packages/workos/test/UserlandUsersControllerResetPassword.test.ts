...
describe("UserlandUsersControllerResetPassword", () => {
  it("resets a user's password using a valid token", async () => {
  ...
  it("fails with BadRequest when token is empty", async () => {
    const error = await runEffect(
      UserlandUsersControllerResetPassword({
        token: "",
        new_password: `Distilled-${testRunId}-Aa1!aaaa`,
  ...
  it("fails with Forbidden when the password fails environment policy", async () => {
  ...
  it("fails with UnprocessableEntity for a password that does not meet policy", async () => {
    const error = await runEffect(
      UserlandUsersControllerResetPassword({
        token: `password_reset_token_${testRunId}`,
        new_password: "1",
