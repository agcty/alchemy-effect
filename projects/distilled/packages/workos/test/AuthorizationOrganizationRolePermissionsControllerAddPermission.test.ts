...
describe("AuthorizationOrganizationRolePermissionsControllerAddPermission", () => {
  it("fails with BadRequest when the role slug is empty", async () => {
  ...
  it("fails with UnprocessableEntity when adding a permission to a built-in environment role", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const org = yield* OrganizationsControllerCreate({
