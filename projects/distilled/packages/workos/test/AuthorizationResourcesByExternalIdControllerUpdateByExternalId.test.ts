...
describe("AuthorizationResourcesByExternalIdControllerUpdateByExternalId", () => {
  it("updates an authorization resource by external id", async (ctx) => {
  ...
  it("fails with BadRequest when the external id is empty", async () => {
  ...
  it("fails with Conflict when the update produces a duplicate external id", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const org = yield* OrganizationsControllerCreate({
  ...
  it("fails with UnprocessableEntity when the resource type slug is invalid", async () => {
    const error = await runEffect(
      Effect.gen(function* () {
        const org = yield* OrganizationsControllerCreate({
