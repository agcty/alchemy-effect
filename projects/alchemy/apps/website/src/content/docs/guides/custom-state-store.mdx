...
import { State, type StateService } from "@oddlynew/alchemy/State";
...
- **Use `encodeState` / `reviveState`** from `@oddlynew/alchemy/State` when
...
import * as Effect from "effect/Effect";
import { State } from "@oddlynew/alchemy/State";
import { postgresState } from "../src/postgres-state.ts";
...
- **FQNs are arbitrary strings** containing `/` and other
  ...
  `encodeFqn` / `decodeFqn` from `@oddlynew/alchemy/FQN`.
