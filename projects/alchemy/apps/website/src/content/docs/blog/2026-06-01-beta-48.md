...
category: release
excerpt: v2.0.0-beta.48 adds a `Cloudflare.Zone` resource, runtime Tunnel bindings, a `RateLimit` binding, and an `alchemy cloudflare create-token` CLI command for minting scoped API tokens.
---
...
```typescript
import * as AdoptPolicy from "@oddlynew/alchemy/AdoptPolicy";
...
- `Cloudflare.TunnelRead` — `get`, `list`, `getToken`, `getConfiguration`
- `Cloudflare.TunnelWrite` — `create`, `update`, `delete`, `putConfiguration`
...
```typescript
const token = yield* Cloudflare.AccountApiToken(`${worker}Token`);
