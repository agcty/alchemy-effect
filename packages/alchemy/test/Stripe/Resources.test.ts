...
describe.sequential("Stripe resources", () => {
  test(
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(
          fake.createIdempotencyKeys.every((key) => key.length <= 255),
        ).toBe(true);
        expect(
        ...
        expect([...fake.prices.values()].every((row) => !row.active)).toBe(
          true,
        );
      }),
  );
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(
          fake.products.get("prod_generated_previous")?.metadata.plan,
        ).toBe("pro");
      }),
  );
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        expect(
          fake.webhookEndpoints.get(endpoint.id)?.metadata.alchemy_id,
        ).toBe("NamespaceA/BillingWebhook");
      }),
  );
  ...
  test.provider(
    "rejects inactive features before product attachment",
    (stack) =>
      Effect.gen(function* () {
        fake.reset();
        fake.features.set(
